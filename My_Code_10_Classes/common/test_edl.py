...
def get_dataset(data_config, split='test', filter_classes=None):
    ...
    if data_type == 'raw_signal':
        ...
        raise ValueError(f"test_edl.py 目前仅支持 'raw_signal' 和 'wpt' 数据类型，当前为: {data_type}")
...
def main():
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:
            ...
            # EDL不确定性计算（直接复制参考代码的计算方式）
            evidence = relu_evidence(outputs)
            alpha = evidence + 1
            S = torch.sum(alpha, dim=1, keepdim=True)
            
            # 计算概率：Dirichlet分布的期望 E[p] = alpha / S
            probs = alpha / S
            
            # 计算不确定性：u = num_classes / S
            uncertainties = num_classes / S
            
            # 获取预测类别（最大概率对应的类别）
            _, preds_local = torch.max(probs, 1)
            
            # 将局部索引映射回原始类别索引
            preds = []
            for i, (pred_local, uncertainty_val) in enumerate(zip(preds_local, uncertainties)):
                uncertainty_val = uncertainty_val.item()
                all_uncertainties.append(uncertainty_val)
                
                # 如果不确定性超过阈值，标记为未知类
                if uncertainty_val > uncertainty_threshold:
                    preds.append(-1)  # -1 表示未知类
                else:
                    # 预测为已知类，需要映射回原始类别ID
                    if pred_local < len(known_classes):
                        preds.append(known_classes[pred_local])
                    else:
                        preds.append(-1)  # 异常情况，标记为未知
